...
public class ClientActionThread implements Runnable {
	...
	final String path = "/Users/yingjianwu/Documents/Java/LeetcodePractice/linkedList/PPTextPartA/src/";
...
//		}
		if (!map.containsKey(fileName) || map.get(fileName) == null || map.get(fileName).size() == 0) {
			System.out.println("You donot have the access of lists of Clients that have this object");
			return null;
		} else {
			List<BrokerObject> l = map.get(fileName);
		...
		try {
			...
			if (toReceive == null) {
				...
				if (!toReceive.isReAndUnRe()) {
					...
					if (l.size() == 0 ) {
						System.out.println("You donot have any server to communicate with for this file");
					}
	...
	private boolean fileExist(String fileName) {
		...
		File file = new File("/Users/yingjianwu/Documents/Java/LeetcodePractice/linkedList/PPTextPartA/src/"+fileName);
